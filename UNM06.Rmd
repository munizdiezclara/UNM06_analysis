...
library(emmeans)
library("writexl")
load("C:/Users/munizdie/OneDrive - Lancaster University/Experiments/UNM06_analysis/UNM06_proc_data.RData")
...
```{r, echo=FALSE}
ggplot(MA_training) +
  geom_point(mapping = aes(x = block, y = mean_accuracy, color = condition)) +
  geom_line(mapping = aes(x = block, y = mean_accuracy, color = condition)) +
...
Subjects in the certain group showed higher accuracy through training than the uncertain group, also showing a consistent increase as block progressed, reaching an accuracy of around 0.9 in block 4. However, the uncertain group showed an increase from block 1 to 2, but kept stable after, with an acuracy of around 0.6 in block 4. A mixed methods ANOVA confirmed that both the effect of the Block and the Condition was significant, as well as the interaction between them. ( respectively,`r apa(ANOVA_resp, effect = "block")`; `r apa(ANOVA_resp, effect = "condition")`; `r apa(ANOVA_resp, effect = "block:condition")`). However, its worth noting that bayesian evidence was very strong in favour of the main effects, but only anecdotal for the interaction (respectively, `r report_BF_and_error(bay_ANOVA_resp[1])`; `r report_BF_and_error(bay_ANOVA_resp[2])`; `r report_BF_and_error(bay_ANOVA_resp_int[1])`). 
...
##Block by Condition Interaction
resp_fitted_interaction <- emmeans(ANOVA_resp, c("block", "condition"))
print(resp_fitted_interaction)
##pairwise comparison with Bonferroni correction.
pairs(resp_fitted_interaction, adjust = "bon")
```
...
```
There are no significant differences in memory due to the condition, the predictiveness or the interaction of them, and the bayesian analysis indicates anecdotal evidence for the null hypothesis in all cases (respectively, `r apa(ANOVA_mem_test, effect = "condition")`, `r report_BF_and_error(bay_ANOVA_mem_test[1])`; `r apa(ANOVA_mem_test, effect = "predictiveness")`, `r report_BF_and_error(bay_ANOVA_mem_test[2])`; `r apa(ANOVA_mem_test, effect = "condition:predictiveness")`, `r report_BF_and_error(bay_ANOVA_mem_test_int[1])`).
...
```
There are no significant differences in memory due to the condition, the predictiveness or the interaction of them, and the bayesian analysis indicates anecdotal evidence for the null hypothesis in all cases (respectively, `r apa(ANOVA_c_mem_test, effect = "condition")`, `r report_BF_and_error(bay_ANOVA_c_mem_test[1])`; `r apa(ANOVA_c_mem_test, effect = "predictiveness")`, `r report_BF_and_error(bay_ANOVA_c_mem_test[2])`; `r apa(ANOVA_c_mem_test, effect = "condition:predictiveness")`, `r report_BF_and_error(bay_ANOVA_c_mem_test_int[1])`).
...
```
There are no differences in accuracy due to predictiveness, and the bayesian analysis indicates anecdotal evidence for the null hypothesis (`r apa(t.test_acc_cert_test)`, `r report_BF_and_error(bay_t.test_acc_cert_test[1])`).
...
There are no significant differences in memory due to predictiveness and the bayesian analysis indicates anecdotal evidence for the null hypothesis (`r apa(t.test_mem_cert_test)`, `r report_BF_and_error(bay_t.test_mem_cert_test[1])`).
...
```
```{r, include=FALSE}
t.test_cms_cert_test <- t.test(mem ~ predictiveness, data = cms_cert_test, paired = TRUE)
print(t.test_cms_cert_test)
...
```
There are no significant differences in memory due to predictiveness  and the bayesian analysis indicates anecdotal evidence for the null hypothesis (`r apa(t.test_cms_cert_test)`, `r report_BF_and_error(bay_t.test_cms_cert_test[1])`).
...
```
There are no significant differences in memory due to predictiveness  and the bayesian analysis indicates anecdotal evidence for the null hypothesis (`r apa(t.test_c_mem_cert_test)`, `r report_BF_and_error(bay_t.test_c_mem_cert_test[1])`).